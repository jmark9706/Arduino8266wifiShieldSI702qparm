 /************************************************************
October 11, 2020 Modified to test query parms wiht flask
Modified to add the I2C sensors
...
const char mySSID[] = "cirrus";
const char myPSK[] = "7138982048";
...
// HTTP Strings //
//////////////////
const char destServer[] = "10.0.0.151";
const String htmlHeader = "HTTP/1.1 200 OK\r\n"
                          "Content-Type: text/html\r\n"
                          "Connection: close\r\n\r\n"
                          "<!DOCTYPE HTML>\r\n"
                          "<html>\r\n";
// modified for sending qparms
const String httpRequest = "GET /parms?temp=33&humidity=45  HTTP/1.1\n"
                           "Host: 10.0.0.151\n"
                           "Connection: close\n\n";
...
void setup() 
{
  ...
  // debug information.
  Serial.begin(9600);
  serialTrigger(F("***** START Press any key to begin."));
...
x21:
  ...
  // read the sensors
  Serial.println(F("Read the sensors"));
  getWeather();
  printInfo();
  // send GET to web server
  clientDemo();
...
void initializeESP8266()
{
  ...
  // true
    Serial.println(F("======> Starting init for the ESP8266."));
...
void connectESP8266()
{
  ...
  { // If it's not in station mode.
    ...
    if (retVal < 0)
    {
      Serial.println(F("Error setting station mode."));
      errorLoop(retVal);
    }
  }
  ...
  x22:  retVal = esp8266.connect(mySSID, myPSK);
    if (retVal < 0)
    {
      Serial.print(F("Error connecting: retry "));
      Serial.print(Retry); Serial.print(" Error code "); Serial.println(retVal);
      if (Retry-- > 0) { delay (100); goto x22;}
     // errorLoop(retVal);
    }
  }
}
...
void clientDemo()
{
  ...
  Serial.print(F("Web server connect code: "));
  Serial.println(retVal);
  if (retVal <= 0)
  {
    Serial.print(F("Failed to connect to web server: "));
    Serial.print(destServer);
    return;
  }
  Serial.println(F("Sending HttpRequest"));
...
// code stolen from  Si7021 example
//---------------------------------------------------------------
void getWeather()
{
  Serial.println(F("Get humidity and temperature"));
